...
public:
  ...
  // inorder traversal print w/o recursion
  void printInorder()
  {
      BSTNode<Key, E>* curr = root;

      while (curr->getLeftThread() == 1) {
          curr = curr->left();
      }

      // now keep traversing the next child node and print it's value
      while (curr != NULL) 
      {
          cout << curr->element() << endl;

          // if node has no current right pointer, adapted for threaded
          if (curr->getRightThread() == 0) 
          {
              curr = curr->right(); // traverse inorder
          }

          // else...
          else 
          {
              // continue to right sub tree
              curr = curr->right();

              // traverse to leftmost node
              while (curr->getLeftThread() != 0) 
              {
                  curr = curr->left();
              }
          }
  ...
  void printReverse()
  {
      ...
      while (curr != NULL) {
          ...
          // if node has no current right pointer, adapted for threaded
          if (curr->getLeftThread() == 0) {
              curr = curr->left(); // traverse inorder
          }
...
// Print out a BST
// study note: Inorder traversal
template <typename Key, typename E>
void BST<Key, E>::
printhelp(BSTNode<Key, E>* root, int level) const {
  if (root == NULL) return;           // Empty tree
  
  // added check for threads
  if(root->getLeftThread()) printhelp(root->left(), level+1);   // Do left subtree
  ...
  // added check for threads
  if(root->getRightThread()) printhelp(root->right(), level+1);  // Do right subtree
}
